...
void check_duplicates(Node *head) {
    ...
    if (count == 0) { 
        ...
        cout << "Total duplicate values: " << count << endl;
    }
}
...
void insert_at_position(Node *&head, int pos, int val) { 
    ...
    if (pos == 0) { // If position is 0, insert at the head
        newNode->next = head;
        head = newNode;
        cout << "Inserted at head: " << val << endl;
        return;
...
void sort_list(Node *&head) {
    if (head == NULL || head->next == NULL) { // Check if the list is empty or has one element
    ...
    while (temp != NULL) {
        ...
        while (innerTemp != NULL) {
            if (temp->val > innerTemp->val) { // Swap values if the current node's value is greater
                swap(temp->val, innerTemp->val);
                swapped = true; // Set flag to true when a swap occurs
            }
            innerTemp = innerTemp->next; // Move to the next node in the inner loop
        }temp = temp->next; // Move to the next node in the outer loop
    }
    if (swapped) {  // Output message based on whether any swaps were made
...
int main() {
    ...
    cout << "5: Insert at position with query\n";
    cout << "6: Sort the linked list\n";
    cout << "7: Terminate linked list operations\n";
    ...
    while (true) {
        ...
        if (op == 1) {
            ...
            while (query > 0) {
                ...
                insert_at_position(head, pos, val); // Insert at the specified position
            ...
            sort_list(head); // Option to sort the linked list
            print_linked_list(head);
            cout << endl << endl;
        } else if (op == 7) {
